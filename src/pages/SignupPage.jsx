...
function SignupPage() {
  const { currentUser, signup, isFirebaseConfigured, firebaseConfigError } = useAuth();
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      setError('Unable to create account. Please try with a different email.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="page-shell flex min-h-[75vh] items-center justify-center">
      <section className="card w-full max-w-md">
        ...
        <p className="mt-4 text-sm text-slate-600">
          ...
          <Link className="font-semibold text-med-700" to="/login">
            Login here
          </Link>
        </p>
      </section>
